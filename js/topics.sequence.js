...
/* -----------------------------------------------------------
   KRÁSNY, ŠTRUKTUROVANÝ SUMÁR OKRUHU (VYSVETLENIE / PRÍKLAD / KĽÚČOVÉ / ZAPAMÄTAJ SI)
----------------------------------------------------------- */
async function showSummary(topic, body, footer) {
  return new Promise(resolve => {

    const raw = topic.summary || "";
    const sections = {
      vysvetlenie: "",
      priklad: "",
      klucove: "",
    ...
    let current = "vysvetlenie";

    raw.split("\n").forEach(line => {
      const l = line.trim().toLowerCase();

      if (l.startsWith("vysvetlenie")) current = "vysvetlenie";
      else if (l.startsWith("príklad") || l.startsWith("priklad")) current = "priklad";
      else if (l.startsWith("kľúčové") || l.startsWith("klucove")) current = "klucove";
      else if (l.startsWith("zapamätaj") || l.startsWith("zapamataj")) current = "zapamataj";

      sections[current] += line + "\n";
    });

    const block = (icon, title, content) => `
      <div style="
        background: linear-gradient(180deg, var(--card), #eef1f6);
      ...
      ">
        ...
        <div style="line-height: 1.55; font-size: 15px; white-space: pre-line;">
          ${content.trim()}
        </div>
      </div>
    `;
    ...
    body.innerHTML = `
      <div style="padding: 16px; font-size: 16px;">
        <h2 style="margin-bottom: 18px; font-size: 22px; font-weight: 900; letter-spacing: -0.5px;">
          Sumár okruhu
        </h2>

        ${block("📘", "Vysvetlenie", sections.vysvetlenie)}
        ${block("🧩", "Príklad", sections.priklad)}
        ${block("🔑", "Kľúčové slová", sections.klucove)}
        ${block("⭐", "Zapamätaj si", sections.zapamataj)}
      </div>
    `;

    footer.innerHTML = `
      <button class="primary-btn click-anim" id="summaryNextBtn">Ďalej</button>
    `;

    document.getElementById("summaryNextBtn").onclick = () => resolve();
  });
}

/* -----------------------------------------------------------
   SEKVENCIA: SUMÁR → KVÍZ → DLAŽDICE → PRÍPADY
----------------------------------------------------------- */
async function runSequence(topic, opts = {}) {
  ...
  header.innerHTML = `<div class="sequence-title">${topic.title || topic.id || "Okruh"}</div>`;
  ...
  try {
    await showSummary(topic, body, footer);
    ...
    footer.querySelector("#seqCloseBtn").onclick = () => {
      try { document.body.removeChild(overlay); } catch (e) {}
    };

    const duelPackage = {
      subject: topic.title || topic.id || "Pracovné právo – štátnice",
      structure: "pracovne_pravo",
    ...
    localStorage.setItem("lastDuelPackage", JSON.stringify(duelPackage));
...
async function loadTopics() {
  ...
  const topicIds = Array.from({ length: 50 }, (_, i) => `A${i + 1}`);
  const topics = [];
