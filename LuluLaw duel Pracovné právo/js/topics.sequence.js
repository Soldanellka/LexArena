...
async function showSummary(topic, body, footer) {
  return new Promise(resolve => {
    body.innerHTML = `
      <div style="padding: 16px; line-height: 1.6; font-size: 16px;">
        <h2 style="margin-bottom: 12px; font-size: 20px; font-weight: 800;">Sumár okruhu</h2>
        <div>${topic.summary || "Sumár nie je dostupný."}</div>
      </div>
    `;
...
async function runSequence(topic, opts = {}) {
  ...
  try {
    ...
    /* 🔥 ÚVODNÝ SUMÁR – PRVÝ KROK */
    await showSummary(topic, body, footer);

    /* 🔥 POTOM KVÍZ → DLAŽDICE → PRÍPADY */
